# -*- coding: utf-8 -*-
"""
Created on Fri Jan 17 07:01:35 2020
...
class TES_wo_damping(Model):
    def __init__(self, model_name):
        super().__init__(model_name)
        
    def model(self, column_name,df, apply_smoothing, trend, seasonal, smoothing_level=None,smoothing_slope=None
              ,smoothing_seasonal=None, seasonal_periods=53):
    
        """
        performs predictions using the triple exponential smoothing without damping model approach
    ...
    def ADE_base_model(self, column_name,df, apply_smoothing, trend, seasonal, smoothing_level=None,smoothing_slope=None,smoothing_seasonal=None, seasonal_periods=53):
...
#        seasonal = ['multiplicative', 'additive']
        ...
        hyperparams = [(asi, sli, ssi, ssei, ti, si) for asi in apply_smoothing for sli in smoothing_levels for ssi in smoothing_slopes for ssei in smoothing_seasonals for ti in trend for si in seasonal]
        hyperparams = [(False, None, None, None, ti, si) if not asi else (asi, sli, ssi, ssei, ti, si) for (asi, sli, ssi, ssei, ti, si) in hyperparams]
        hyperparams = list(set(hyperparams))
        
        hyperparam_combinations = [{'apply_smoothing':asi, 'smoothing_level':sli, 'smoothing_slope':ssi, 'smoothing_seasonal': ssei, 'trend':ti, 'seasonal':si} for (asi, sli, ssi, ssei, ti, si) in hyperparams]
...
#        hyperparams.append((False,None,None,None,'additive','additive'))
#        hyperparams.append((False,None,None,None,'multiplicative','additive'))
#        hyperparams.append((False,None,None,None,'additive','multiplicative'))
#        hyperparams.append((False,None,None,None,'multiplicative','multiplicative'))
