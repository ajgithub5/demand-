# -*- coding: utf-8 -*-
"""
Created on Fri Jan 17 07:20:36 2020
...
class DES_wo_damping(Model):
    def __init__(self, model_name):
        super().__init__(model_name)
    
    def model(self, column_name, df, apply_smoothing, smoothing_level=None,smoothing_slope=None):
        ...
        if apply_smoothing == True:
            fit1 = Holt(df["train"][:-m], damped=False).fit(smoothing_level = smoothing_level, smoothing_slope = smoothing_slope, optimized = True)
            params = None
    ...
    def ADE_base_model(self, column_name, df, apply_smoothing, smoothing_level=None,smoothing_slope=None):
...
#        smoothing_slopes = np.arange(0,1,0.1)
        ...
        hyperparams = [(asi, sli, ssi) for asi in apply_smoothing for sli in smoothing_levels for ssi in smoothing_slopes]
        hyperparams = [(False, None, None) if not asi else (asi, sli, ssi) for (asi, sli, ssi) in hyperparams]
        hyperparams = list(set(hyperparams))
        print(f'hyperparams: {hyperparams}')
        
        #hyperparams = get_random_grid(hyperparams, random_seed=0, sample_size=0.25)
        #hyperparams.append((False,None,None))
        #print(f'random hyperparams: {hyperparams}')
        hyperparam_combinations = [{'apply_smoothing':asi, 'smoothing_level':sli, 'smoothing_slope':ssi} for (asi,sli,ssi) in hyperparams]
    ...
    #Main wrapper for sma model train (gridsearch) and test on cv splits
