...
class WMA12(Model):
    ...
    def model(self, column_name, df, nweeks):
        ...
        weights = []
        for x in range(df.shape[0])[1:13]:
            weights.append(x)
        weights = [x/sum(weights) for x in weights]
        new_df = df[-(12+m):]
        
        new_df["weights"] = np.nan
        new_df["weights"].iloc[:12] = weights
        ...
        df[column_name].iloc[-(12+m):] = new_df[column_name]
        df[column_name].iloc[:-(12+m)] = df['train'].iloc[:-(12+m)]
