    ...
    def model(self, column_name, input_dfs, df_daily, team_location, base_models):
        ...
        for bl,bo,l,ml in zip(base_learners,base_optimal_hyperparams,base_logics,meta_learners):
            try:
                ...
                base_mapes.append(accuracy_metrics(dfo['test'][-m-self.omega:-m],dfo[column_name][-m-self.omega:-m])[0])
        ...
        for bl,bo,l,ml in zip(sorted_base_learners, sorted_base_optimal_hyperparams, sorted_base_logics, sorted_meta_learners):
            ...
            print('====')
            print(bl)
            for i in (dfo[column_name][self.K:-m]-dfo['test'][self.K:-m]):
                if np.isnan(i): 
                    print('Numerator is NaN: {}'.format(i))
                    (dfo[column_name][self.K:-m]-dfo['test'][self.K:-m]).to_csv('baseline.csv')
            for i in dfo['test'][self.K:-m]:
                if i==0: print('Denominator is zero')
                if np.isnan(i): print('Denominaator is NaN: {}'.format(i))
            print('====')
            '''
            ml.fit(
        ...
        for j in range(m):
            softmax_weights = self.apply_softmax(df_ADE.iloc[-m+j][['Meta_Predictions'+'_{}'.format(i) for i in range(self.top_n)]].tolist())
